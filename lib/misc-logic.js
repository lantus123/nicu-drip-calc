...
(function (root, factory) {
  ...
  // 有些藥只能以 vial／amp 為單位開立，算完 mg 還要換算成該單位
  function orderUnitFor(drug, amt) {
    var sup = drug && drug.supply;
    if (!sup || !amt || amt.unit !== 'mg' || !(sup.perUnitMg > 0)) return null;
    return {
      label: sup.label, note: sup.note || null, required: !!sup.orderInUnit,
      min: amt.min / sup.perUnitMg, max: amt.max / sup.perUnitMg,
      perUnitMg: sup.perUnitMg,
    };
  }
  ...
  function compute(drug, weightG) {
    ...
    var rows = drug.regimens.map(function (r) {
      ...
      amt.orderUnit = orderUnitFor(drug, amt);
      return amt;
    });
  ...
  function explain(drug, weightG) {
    ...
    out.rows.forEach(function (r) {
      ...
      if (r.orderUnit) {
        steps.push({
          label: r.orderUnit.required ? '開立單位' : '換算單位',
          detail: rangeText(r.min, r.max) + ' ' + r.unit + ' ÷ ' + r.orderUnit.perUnitMg + ' mg/' + r.orderUnit.label
            + (r.orderUnit.note ? '（' + r.orderUnit.note + '）' : ''),
          value: rangeText(r.orderUnit.min, r.orderUnit.max) + ' ' + r.orderUnit.label,
          emphasis: r.orderUnit.required,
        });
      }
  ...
  return { amountFor: amountFor, volumeFor: volumeFor, orderUnitFor: orderUnitFor, compute: compute, reviewOrder: reviewOrder,
