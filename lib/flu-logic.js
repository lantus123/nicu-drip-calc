...
(function (root, factory) {
  ...
  // 計算過程：每一行都要能被獨立驗算
  function explain(D, out, weightG) {
    if (!out || !out.ok) return [{ label: '無法計算', detail: '', value: (out && out.reason) || '' }];
    var kg = weightG / 1000;
    var steps = [{ label: '體重', detail: '輸入值', value: weightG + ' g = ' + fmt(kg) + ' kg' }];
    if (out.bandLabel) steps.push({ label: 'interval', detail: out.bandLabel, value: out.interval });
    if (out.renalNote) steps.push({ label: '腎功能調整', detail: out.renalNote, value: '× ' + D.renal.factor });
    out.rows.forEach(function (r) {
      steps.push({
        label: r.label,
        detail: rangeText(r.perKg.min, r.perKg.max) + ' mg/kg × ' + fmt(kg) + ' kg'
          + (out.renalNote ? ' × ' + D.renal.factor : ''),
        value: rangeText(r.amount.min, r.amount.max) + ' mg' + (r.interval ? ' ' + r.interval : ''),
        emphasis: true,
      });
      if (r.amount.rounded) steps.push({
        label: '無條件進位', detail: out.roundUpBasis || '進位至整數',
        value: rangeText(r.amount.rounded.min, r.amount.rounded.max) + ' mg',
      });
      if (r.infusion) {
        steps.push({
          label: '稀釋容積', detail: fmt(r.amount.max) + ' mg ÷ ' + D.administration.maxConcentration + ' mg/mL',
          value: fmt(r.infusion.volumeMl) + ' mL',
        });
        steps.push({
          label: '最短輸注', detail: r.infusion.limitedBy, value: fmt(r.infusion.minHours) + ' 小時',
        });
      }
    });
    return steps;
  ...
  return { intervalFor: intervalFor, renalFactor: renalFactor, infusion: infusion,
           ...
           computeIndication: computeIndication, reviewOrder: reviewOrder, explain: explain };
}));
