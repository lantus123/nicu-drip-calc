...
const orphan = [...ids].filter(id => /^(abx|misc)/.test(id) && !used.has(id));
check('abx / misc 的 id 都有被腳本使用', orphan.length === 0, orphan.join(', '));

check('分頁按鈕齊全', (html.match(/data-tab="/g) || []).length === 3, '應有 3 個');
check('三個面板都在', ['panel-drip','panel-abx','panel-misc'].every(i => ids.has(i)), '缺面板');
check('資料檔在邏輯檔之前載入',
